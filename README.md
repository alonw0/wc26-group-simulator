...
- **Two release cadences, because there are two artifact kinds.** The *image* ships on every merge to `main`, docs included — it's the app, and in continuous delivery every commit on main is deployable and deployed. The *libraries* publish only when their packed content actually changed ([`scripts/publish-if-changed.sh`](scripts/publish-if-changed.sh) compares content hashes against the latest published version, ignoring the stamped version field). A library version is a signal to consumers; republishing identical bits under a new number generates Dependabot PRs and `npm outdated` noise for a diff of zero. Versions `0.1.3`–`0.1.8` of the engine are byte-identical for exactly this reason — they predate the guard, and they're left in the registry as the visible moment this lesson was learned.
- **`0.1.<run>` versioning instead of semver.** Versions here encode *traceability*, not compatibility: any published version maps 1:1 to the CI run that produced it. Artifacts no longer share lockstep numbers (the image may be `0.1.12` while the engine is still `0.1.9`) — and that's fine, because **the release is the grouping mechanism**: Fly ties whatever a run actually shipped into one release, which is the answer to "what shipped together" that version-number lockstep was faking. Semver earns its keep when strangers depend on your public API; that's the documented next step, not the demo's. Re-runs of a failed workflow get a `-rerun.N` suffix so a retry can never collide with an already-published immutable version.
